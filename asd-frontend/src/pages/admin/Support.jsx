...
//                 <p className="text-sm sm:text font-semibold text-gray-700 mb-2">
//                   Roles
//                 </p>
...
//                   <p className="text-sm  sm:text-lg font-semibold text-gray-700 mb-3">
//                     1. Roles Information
//                   </p>
//                   <div className="space-y-3">
//                     <div>
//                       <label className="block text-xs  sm:text-lg  text-[#767676] mb-1">
//                         Role Name
//                       </label>
//                       <input
//                         type="text"
...
//                         className="w-full border border-gray-300 rounded-lg px-3 py-2 text-lg text-[#767676] focus:outline-none focus:ring-2 focus:ring-teal-300 disabled:bg-gray-50 disabled:text-gray-400"
//                       />
//                     </div>
//                     <div>
//                       <label className="block text-xs sm:text-lg text-[#767676] mb-1">
...
//                         disabled={selected.core}
//                         rows={2}
//                         className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-[#767676] focus:outline-none focus:ring-2 focus:ring-teal-300 resize-none disabled:bg-gray-50 disabled:text-gray-400"
//                       />
//                     </div>
//                   </div>
//                 </div>
...
//                 <div className="bg-white border border-gray-200 rounded-xl p-4 ">
//                   <p className="text-sm sm:text-lg font-semibold text-[#767676] mb-3">
//                     1. Permissions
//                   </p>
...
import {  FaChevronRight, FaRightLeft} from 'react-icons/fa6'
...
export default function Support() {
  const [departments, setDepartments] = useState([{name: 'd', description:"des", type:'', members:[], status: "active", roles: [] },{name: 'dd', description:"des", type:'', members:[], status: "active", roles: [] }, {name: 'd', description:"des", type:'', members:[], status: "active", roles: [] }, ]);
  const [view, setView] = useState("list");
  const [editDept, setEditDept] = useState(null);
    ...
    return (
      <div className="min-h-screen bg-gray-50 px-3 py-6 sm:px-6 sm:py-8 ">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-start justify-between mb-5">
            <div>
              <div className="flex items-center gap-1    mb-1">
                <button onClick={() => setView("list")} className="font-bold text-sm sm:text-xl  lg:text-3xl text-gray-800 hover:text-teal-500 transition-colors">Support</button>
               <FaChevronRight  className="text-lg text-black"/>
                <span className="text-lg  font-medium ">Create Department</span>
              </div>
              <p className="text-sm sm:text-lg text-[#575757]">Update department details and team members</p>
            </div>
            <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 text-sm  sm:text-lg font-medium text-white bg-teal-500 hover:bg-teal-600 rounded-lg transition-colors flex-shrink-0">
              + Create Department
            </button>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-5">
            <div>
              <h3 className="text-sm sm:text-xl  xl:text-2xl font-medium   text-[#0F8A7D] mb-4">Basic Information</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm sm:text-lg font-medium mb-1.5">Department Name</label>
                  <input
                    type="text"
                    value={form.name}
                    onChange={(e) => handleChange("name", e.target.value)}
                    placeholder="Sales Department"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm sm:text-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-300"
                  />
                </div>
                <div>
                  <label className="block text-sm sm:text-lg font-medium   mb-1.5">Description</label>
                  <textarea
                    value={form.description}
                    onChange={(e) => handleChange("description", e.target.value)}
                    placeholder="Drives revenue by managing client relationships generating leads, and closing deals."
                    rows={3}
                    className="w-full border sm:text-lg border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-300 resize-none"
                  />
                </div>
                <div>
                  <label className="block text-sm  sm:text-lg font-medium   mb-1.5">Department Type</label>
                  <div className="relative">
                    <select
                      value={form.type}
                      onChange={(e) => handleChange("type", e.target.value)}
                      className="w-full  border border-gray-300 rounded-lg px-3 py-2.5 text-sm sm:text-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-300 appearance-none bg-white"
                    >
                  ...
                  <div className="mt-2 bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 flex items-center gap-2">
                    ...
                    <span className="text-xs sm:text-lg text-yellow-700">Changing department type may affect role suggestions</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-sm sm:text-xl  xl:text-2xl font-medium  text-teal-600 mb-3">Team Members</h3>
              <label className="block text-sm sm:text-lg font-medium   mb-1.5">Add Team Members</label>
              <div className="relative mb-3">
                <FiSearch className="absolute left-3 top-1/2 text-sm sm:text-xl -translate-y-1/2 text-gray-400"  />
                <input
                  type="text"
                  value={memberSearch}
                  ...
                  onFocus={() => setMemberDropOpen(true)}
                  placeholder="Search users by name or email..."
                  className="w-full pl-9 pr-9 py-2.5 border border-gray-300 rounded-lg text-sm sm:text-lg text-gray-600 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-300"
                />
    ...
    {u.name}
              ...
              {form.members.length > 0 && (
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => handleChange("members", [])} className="text-xs text-[#14B8A6]">Clear</button>
                </div>
              )}

              <div className="border border-dashed border-gray-300 rounded-lg py-4 text-center">
                <p className="text-sm sm:text-lg lg:text-xl text-[#707071]">No members assigned</p>
              </div>
            </div>

            <div className="flex items-center justify-between pt-1">
              <span className="text-sm  sm:text-xl font-medium text-[#14B8A6]">Status</span>
              <Toggle checked={form.status} onChange={(val) => handleChange("status", val)} />
            </div>
          </div>

          <div className="flex gap-3 mt-4">
            <button onClick={() => setView("list")} className="flex-1 py-3 text-sm  sm:text-xl font-medium border border-gray-300 rounded-lg text-[#C4C4C4] hover:bg-gray-50 transition-colors">
              Cancel
            </button>
            <button onClick={handleSave} className="flex-1 py-3 text-sm sm:text-xl font-medium text-white bg-teal-500 hover:bg-teal-600 rounded-lg transition-colors">
              Save Changes
            </button>
          </div>
          <p className="text-xs sm:text-xl text-[#888888] text-center mt-2">You have unsaved changes</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-3 py-6 sm:px-6 sm:py-8 ">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-start justify-between mb-5">
          <div>
            <h1 className="text-xl sm:text-xl lg:text-3xl xl:text-4xl font-bold text-gray-800">Support</h1>
            <p className="text-xs sm:text-lg text-gray-500 mt-0.5">Manage user rooles and their permissions</p>
          </div>
          <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 text-sm sm:text-base font-medium text-white bg-teal-500 hover:bg-teal-600 rounded-lg transition-colors flex-shrink-0">
            + Create Department
          </button>
        </div>

        <div className="bg-[#BDD9F5] border border-blue-200 rounded-xl px-4 py-5 flex items-start gap-3 mb-5">
          <HiOutlineOfficeBuilding className="text-teal-600 mt-0.5 flex-shrink-0" size={18} />
          <div>
            <p className="text-sm font-semibold text-[#1F73C6]">Department Role Assignment</p>
            <p className="text-xs sm:text-sm text-[#1F73C6] mt-0.5">When creating users, you can assign them to departments. The department's pre-configured roles and permissions will be automatically applied to the user.</p>
          </div>
        </div>
            ...
            <div key={dept._id} className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm sm:text-lg lg:text-2xl font-medium text-gray-800">{dept.name}</p>
                    <p className="text-xs sm:text-sm font-medium text-[#6D6D6D] mt-0.5">{dept.description}</p>
                  </div>
                </div>
                <button className="text-gray-400 hover:text-gray-600 p-1">
                  <FiMoreVertical size={16} />
              ...
              <p className="text-xs sm:text-base text-[#6D6D6D] mb-1.5">Team Members</p>
              <div className="flex items-center gap-1 mb-3">
              {dept.members.slice(0, 4).map((m, i) => (
  <div key={m._id} style={{ marginLeft: i > 0 ? "-6px" : "0" }}>
    <Avatar name={m.name} index={i} />
                ...
                {dept.members.length > 4 && (
                  <span className="text-sm text-[#6D6D6D] ml-2">+{dept.members.length - 4}</span>
                )}
              </div>

              <p className="text-xs sm:text-base text-[#6D6D6D] mb-1.5">Department Roles</p>
              <div className="flex flex-wrap gap-1.5 mb-4">
              {dept.roles.map((r) => (
  <span key={r._id} className="px-2 py-0.5 text-xs bg-teal-50 text-teal-700 rounded-full">
    {r.name}
              ...
              <div className="flex gap-2">
                <button onClick={() => openEdit(dept)} className="flex-1 py-1.5 text-sm sm:text-base  font-medium border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                  Edit
                </button>
                <button   className="flex-1 py-1.5 text-sm sm:text-base font-medium border border-teal-400 text-teal-500 rounded-lg hover:bg-teal-50 transition-colors">
                  Manage Roles
                </button>
              </div>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5">
          <h3 className="text-sm sm:text-lg font-semibold text-gray-800 mb-4">Department Access Control</h3>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[400px] rounded-lg">
              <thead className="px-4">
                <tr className="border-b  border-gray-100 bg-[#E9F2FC] py-2 rounded-2xl px-5 ml-3">
                  <th className="text-left text-xs sm:text-base xl:text-lg font-medium  text-[#1F73C6] pl-4 py-2 pr-4">DEPARTMENT</th>
                  {accessKeys.map((k) => (
                    <th key={k} className="text-center text-xs sm:text-base xl:text-lg   font-medium text-[#1F73C6] py-2 px-2">{k}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {departments.map((dept) => (
                  <tr key={dept._id} className="border-b border-gray-50 last:border-0">
                    <td className="py-1 pl-4 pr-4 text-sm sm:text-lg  lg:text-2xl text-gray-700">{dept.name}</td>
                    {accessKeys.map((k) => (
                      <td key={k} className="py-1 px-2 text-center">
                        <input
                          type="checkbox"
...
onChange={() => toggleAccess(dept._id, k)}
                          className="w-4 h-4 accent-[#0F8A7D] cursor-pointer"
                        />
                      </td>
                    ))}
                  </tr>
